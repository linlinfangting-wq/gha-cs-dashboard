...
.insight-en {{ opacity: 0.72; font-size: 0.78rem; }}
</style>
...
def insight_category(df):
    ...
    if '技术' in cat_s or 'bug' in cat_s.lower():
        zh_a = '技术问题在小红书等开放平台易演变为负面 UGC，建议快速响应并主动公告处理进展'
        en_a = 'Tech issues on open platforms like Xiaohongshu can escalate into negative UGC; quick response and proactive updates recommended'
    elif '需求' in cat_s or 'Feature' in cat_s:
        zh_a = '需求类咨询是产品迭代与内容选题的方向信号，可沉淀为 FAQ 或推文素材'
        en_a = 'Feature requests signal product iteration directions; consider compiling into FAQ or social content'
    elif '会员' in cat_s or 'Benefits' in cat_s:
        zh_a = '权益咨询量高反映信息透明度不足，建议通过内容科普（推文/短视频）降低重复咨询'
        en_a = 'High benefits inquiry volume suggests insufficient transparency; address via educational content (posts/short videos)'
    elif '数据' in cat_s or 'Data' in cat_s:
        zh_a = '数据不一致影响用户决策，需向酒店和平台团队同步跟进，并在内容端说明差异'
        en_a = 'Data inconsistencies affect user decisions; sync follow-up with hotel and platform teams and clarify discrepancies in content'
    else:
        zh_a = '建议通过自助内容（FAQ/教程）前置拦截，降低重复咨询率'
        en_a = 'Self-service content (FAQ/tutorials) can proactively reduce repeat inquiry rates'
    zh = f'<b>{name}</b> 是最高频问题，占 <b>{pct:.0f}%</b>（{cnt} 条）。{zh_a}。'
    en = f'<span class="insight-en"><b>{name}</b> is the top issue at <b>{pct:.0f}%</b> ({cnt} records). {en_a}.</span>'
    return f'{zh}<br>{en}'
...
def insight_channel(df):
    ...
    if '小红书' in ch_s:
        zh_a = '小红书为开放平台，咨询公开可见，建议设定 24h 内响应 SLA 保护品牌形象'
        en_a = 'Xiaohongshu is an open platform where inquiries are publicly visible; set a 24h response SLA to protect brand reputation'
    elif '微信群' in ch_s:
        zh_a = '社群场景中问题对全体成员可见，优先处理可提升社群信任度'
        en_a = 'Issues in group chats are visible to all members; prioritizing responses builds community trust'
    elif '微信' in ch_s:
        zh_a = '私域核心阵地，响应质量直接影响用户留存与口碑传播'
        en_a = 'A core private-domain channel; response quality directly impacts user retention and word-of-mouth'
    else:
        zh_a = '建议持续跟踪该渠道响应速度与用户满意度'
        en_a = 'Continuously monitor response speed and user satisfaction for this channel'
    zh = f'<b>{name}</b> 是主要来源渠道（占 <b>{pct:.0f}%</b>，{cnt} 条）。{zh_a}。'
    en = f'<span class="insight-en"><b>{name}</b> is the top source channel ({pct:.0f}%, {cnt} records). {en_a}.</span>'
    return f'{zh}<br>{en}'
...
def insight_ch_cat(df):
    ...
    if '小红书' in ch_s:
        zh_a = '该路径发生在开放平台，问题可见度高，建议提高响应优先级并在评论区主动引导私信'
        en_a = 'This path occurs on an open platform with high visibility; increase response priority and proactively guide users to DM in comments'
    elif '微信群' in ch_s:
        zh_a = '群内高频问题若积压可能引发集体不满，建议排查根本原因并统一回复'
        en_a = 'Unresolved high-frequency group issues can trigger collective dissatisfaction; investigate root cause and issue unified responses'
    else:
        zh_a = '优化该路径的标准应答模板可大幅提升整体响应效率'
        en_a = 'Optimizing standard response templates for this path can significantly improve overall efficiency'
    zh = f'最高频路径 / Top path：<b>{ch_s} → {cat_s}</b>（<b>{len(flow)}</b> 条 records）。{zh_a}。'
    en = f'<span class="insight-en">{en_a}.</span>'
    return f'{zh}<br>{en}'
...
def insight_trend(df):
    ...
    if len(monthly) < 2:
        zh = f'当前共 <b>{len(df)}</b> 条记录，数据积累中，建议持续记录以观察趋势规律。'
        en = f'<span class="insight-en">Currently <b>{len(df)}</b> records. Data accumulating — continue logging to observe trend patterns.</span>'
        return f'{zh}<br>{en}'
    ...
    if diff > 0:
        ...
        zh_a = '咨询量增长可能与近期推广活动或新用户涌入相关，建议核查活动节点'
        en_a = 'The increase may be linked to recent promotions or new user influx; cross-check with campaign timelines'
    elif diff < 0:
        zh_dir = f'下降 <b>{abs(diff)}</b> 条'
        en_dir = f'decreased by <b>{abs(diff)}</b>'
        zh_a = '咨询量下滑可能是内容教育有效或社区活跃度下降，建议结合社媒数据交叉验证'
        en_a = 'The decline may reflect effective content education or lower community activity; cross-validate with social media data'
    else:
        zh_dir = '持平'
        en_dir = 'remained flat'
        zh_a = '咨询量稳定，可持续观察高频问题是否被内容/产品侧逐步消化'
        en_a = 'Volume is stable; continue observing whether high-frequency issues are gradually resolved by content or product updates'
    peak = monthly.idxmax()
    zh = f'最近月咨询量较上月{zh_dir}。历史峰值在 <b>{peak}</b>（{monthly.max()} 条）。{zh_a}。'
    en = f'<span class="insight-en">Latest month {en_dir} vs. prior month. Historical peak: <b>{peak}</b> ({monthly.max()} records). {en_a}.</span>'
    return f'{zh}<br>{en}'
...
with st.sidebar:
    ...
    st.markdown('---')
    if st.button('🔄 刷新数据 / Refresh Data', use_container_width=True):
        st.cache_data.clear()
        st.rerun()
...
if df.empty:
    st.warning('当前筛选条件下无数据 / No data for current filters')
    st.stop()
...
with col_a:
    ...
    st.markdown('<div class="card-title">Issue Distribution</div><div class="card-subtitle">问题类别分布 / Issue Category Breakdown</div>', unsafe_allow_html=True)
    ...
    fig.update_traces(
        ...
        hovertemplate='<b>%{label}</b><br>咨询量 / Inquiries: %{value}<br>占比 / Share: %{percentRoot:.1%}<extra></extra>',
...
with col_b:
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="card-title">Channel Source</div><div class="card-subtitle">渠道来源分布 / Inquiry Channel Breakdown</div>', unsafe_allow_html=True)
    ...
    fig = go.Figure(go.Pie(
        ...
        hovertemplate='%{label}<br><b>%{value}</b> 条 records (%{percent})<extra></extra>',
        pull=[0.04] + [0] * (len(ch_df) - 1),
    ))
    fig.add_annotation(text=f'<b>{total}</b><br><span style="font-size:10px">总计<br>Total</span>',
...
# ═══════════════════════════════════════
st.markdown('<div class="card">', unsafe_allow_html=True)
st.markdown('<div class="card-title">Channel × Category Matrix</div><div class="card-subtitle">各渠道问题类别分布 / Issue Distribution by Channel & Category</div>', unsafe_allow_html=True)
...
fig = go.Figure(go.Scatter(
    ...
    hovertemplate='<b>%{x}</b> · <b>%{y}</b><br>数量 / Count: %{customdata}<extra></extra>',
))
fig.update_layout(
...
trend_mode = st.radio(
    '', options=['📅 每月新增 / Monthly New', '📈 累计总量 / Cumulative'],
    horizontal=True, label_visibility='collapsed', key='trend_mode',
)

if trend_mode == '📅 每月新增 / Monthly New':
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="card-title">Monthly New Inquiries</div><div class="card-subtitle">每月新增咨询量 / New Inquiries per Month</div>', unsafe_allow_html=True)
    fig1 = go.Figure()
    fig1.add_trace(go.Bar(
        x=monthly_total['month'], y=monthly_total['count'],
        name='月新增 / Monthly New', marker_color=BLUE, marker_line_width=0,
        hovertemplate='%{x}<br>新增 / New: <b>%{y}</b><extra></extra>',
    ))
    fig1.add_trace(go.Scatter(
        x=monthly_total['month'], y=monthly_total['count'],
        mode='lines+markers+text', name='月总计 / Monthly',
    ...
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="card-title">Cumulative Total</div><div class="card-subtitle">累计咨询量增长 / Cumulative Inquiry Growth</div>', unsafe_allow_html=True)
    fig2 = go.Figure()
    fig2.add_trace(go.Bar(
        x=monthly_total['month'], y=monthly_total['count'],
        name='当月新增 / Monthly New', marker_color=BLUE, marker_line_width=0, opacity=0.35,
        hovertemplate='%{x}<br>当月新增 / New: <b>%{y}</b><extra></extra>',
    ))
    fig2.add_trace(go.Scatter(
        x=monthly_total['month'], y=monthly_total['cumulative'],
        mode='lines+markers+text', name='累计总量 / Cumulative',
        ...
        hovertemplate='%{x}<br>累计 / Cumulative: <b>%{y}</b><extra></extra>',
    ))
    fig2.update_layout(
...
# ═══════════════════════════════════════
st.markdown('<div class="card">', unsafe_allow_html=True)
st.markdown('<div class="card-title">Platform × Category Heatmap</div><div class="card-subtitle">平台与问题类别交叉热力图 / Platform vs. Category Heatmap</div>', unsafe_allow_html=True)
...
if not heat.empty:
    fig = go.Figure(go.Heatmap(
        ...
        hovertemplate='平台 / Platform: %{y}<br>类别 / Category: %{x}<br>数量 / Count: <b>%{z}</b><extra></extra>',
        showscale=True,
        colorbar=dict(thickness=12, len=0.8, title=dict(text='数量<br>Count', side='right')),
    ))
    fig.update_layout(
        paper_bgcolor='white', plot_bgcolor='white',
    ...
    if max_pos:
        pl, cat = max_pos[0]
        zh = f'<b>{pl}</b> 平台的 <b>{cat}</b> 问题最集中（<b>{int(max_val)}</b> 条）。针对该组合定向制作解答内容，可有效减少重复咨询并提升平台口碑。'
        en = f'<span class="insight-en"><b>{pl}</b> platform has the highest concentration of <b>{cat}</b> issues (<b>{int(max_val)}</b> records). Targeted content for this combination can reduce repeat inquiries and improve platform reputation.</span>'
        st.markdown(f'<div class="insight">💡 {zh}<br>{en}</div>', unsafe_allow_html=True)
else:
    st.info('暂无足够数据生成热力图 / Insufficient data for heatmap')
st.markdown('</div>', unsafe_allow_html=True)
...
with col_e:
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="card-title">Top Keywords</div><div class="card-subtitle">高频关键词 / Top Keywords</div>', unsafe_allow_html=True)
    all_kw = []
    ...
    if all_kw:
        ...
        fig = go.Figure(go.Scatter(
            ...
            hovertemplate='<b>%{text}</b><br>出现 / Appeared: %{customdata} 次 times<extra></extra>',
        ))
        fig.update_layout(
            paper_bgcolor='white', plot_bgcolor='white',
        ...
        st.plotly_chart(fig, use_container_width=True)
        top1, cnt1 = top[0]
        zh = f'"<b>{top1}</b>" 是最高频关键词（<b>{cnt1}</b> 次），反映用户核心诉求，可作为内容选题方向优化社媒触达。'
        en = f'<span class="insight-en">"<b>{top1}</b>" is the top keyword (<b>{cnt1}</b> occurrences), reflecting core user needs and suggesting content directions for social media reach optimization.</span>'
        st.markdown(f'<div class="insight">💡 {zh}<br>{en}</div>', unsafe_allow_html=True)
    else:
        st.info('暂无关键词数据 / No keyword data')
    st.markdown('</div>', unsafe_allow_html=True)

with col_f:
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="card-title">Records</div><div class="card-subtitle">数据明细 / All Records</div>', unsafe_allow_html=True)
...
st.markdown(f'<p style="text-align:right;color:#ccc;font-size:0.72rem;margin-top:8px">共 {len(df)} 条记录 / {len(df)} records · 数据每5分钟刷新 / Refreshes every 5 min · GHA Discovery Customer Service</p>', unsafe_allow_html=True)
